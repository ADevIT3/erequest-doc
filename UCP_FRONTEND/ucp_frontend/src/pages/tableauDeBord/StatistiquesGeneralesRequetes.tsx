// StatistiquesGeneralesRequetes.tsx
...
interface FiltresDTO {
    ...
    idsites?: number[];
    idagmos?: number[];
    // For this specific API endpoint (tdb11statrequete), datedu, dateau, statut, numero, refinterne are not used for filtering,
    // but they are part of the common FiltresDTO from the backend. We'll send default values.
    datedu?: string;
    dateau?: string;
    statut?: number | null;
    numero?: string;
...
const StatistiquesGeneralesRequetes: React.FC = () => {
    const [stats, setStats] = useState<StatRequete[]>([]);
    ...
    const fetchStatistiques = async () => {
        ...
        try {
            const response = await axios.post<StatRequete[]>(
                `${API_BASE_URL}/Bord/tdb11statrequete`,
                filters
            );
    ...
    const handleMultiSelectChange = (
        ...
        // This logic handles both adding and removing items for multi-select simulation
    ...
    return (
        <div className="container mx-auto p-4">
            ...
            <h1 className="text-2xl font-bold mb-6">Statistiques générales des requêtes</h1>
                ...
                <Table className="min-w-full bg-white border border-gray-200">
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead className="py-2 px-4 border-b text-right">Initiées</TableHead>
                            <TableHead className="py-2 px-4 border-b text-right">Envoyer</TableHead>
                    ...
                    <TableBody>
                        {dataRows.length > 0 ? (
                            dataRows.map((stat, index) => (
                                <TableRow key={index} className="hover:bg-gray-50">
                                    ...
                                    <TableCell className="py-2 px-4 border-b text-right">{stat.initie}</TableCell>
                        ...
                        {totalRow && (
                            <TableRow className="bg-gray-100 font-bold">
                                ...
                                <TableCell className="py-2 px-4 border-b text-right">{totalRow.initie}</TableCell>
...
export default StatistiquesGeneralesRequetes;
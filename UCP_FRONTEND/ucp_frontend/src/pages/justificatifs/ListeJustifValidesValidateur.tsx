...
const JustifValides: React.FC = () => {
    ...
    const fetchNbJustifsInitiees = async () => {
        ...
        setError(null);
        try {
            let url = "/Justificatif/validateur/valides/pages";

            // Si on a un terme de recherche, utiliser l'endpoint de recherche
            if (debouncedSearchTerm.trim() !== "") {
                url = `/Justificatif/validateur/valides/word/${encodeURIComponent(debouncedSearchTerm)}/pages`;
            }
    ...
    const fetchJustifsInitiees = async () => {
        ...
        setError(null);
        try {
            let url = `/Justificatif/validateur/valides/page/${currentPage}`;

            // Si on a un terme de recherche, utiliser l'endpoint de recherche
            if (debouncedSearchTerm.trim() !== "") {
                url = `/Justificatif/validateur/valides/word/${encodeURIComponent(debouncedSearchTerm)}/page/${currentPage}`;
            }
        ...
        <>
            ...
            <div className="flex flex-1 flex-col gap-4 p-4 bg-[#fafafa]">
                ...
                <div className="rounded-sm border bg-card p-4">
                    {/*<h2 className="text-lg font-semibold mb-4 px-4 py-4">Justificatifs validés</h2>
                                    ...
                                    {justifInitiees.length === 0 ? (
                                        ...
                                        justifInitiees.map((justif) => (
                                            <tr key={justif.idJustif} className="hover:bg-gray-100 cursor-pointer">
                                                ...
                                                {/* <td className="border-b  py-2 text-xs text-zinc-500">{new Date(justif.creationDate).toLocaleDateString()}</td>
                                                
                                                <td className="border-b  py-2 text-xs text-zinc-1000  text-zinc-1000 text-left">{justif.requete.montant?.toLocaleString(undefined, { minimumFractionDigits: 0 })} Ar</td>*/}

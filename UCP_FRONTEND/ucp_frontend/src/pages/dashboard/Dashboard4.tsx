...
const Dashboard1: React.FC = () => {
    const [requetes, setRequetes] = useState<Tdb1RequeteDTO[]>([]);
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await axios.post<Tdb1RequeteDTO[]>(
                '/Bord/tdb1requete',
    ...
    const handleFilterChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        setFiltres(prev => ({
            ...prev,
            [name]: name === "statut" ? (value ? parseInt(value) : null) : value
        }));
    };
    ...
    const getStatusBadge = (statut: string) => {
        const statusMap: { [key: string]: { color: string; label: string } } = {
            'Initié': { color: 'bg-gray-100 text-gray-800', label: 'Initié' },
            'Envoyé': { color: 'bg-blue-100 text-blue-800', label: 'Envoyé' },
            ...
            'Clôturé': { color: 'bg-purple-100 text-purple-800', label: 'Clôturé' }
        };

        const status = statusMap[statut] || { color: 'bg-gray-100 text-gray-800', label: statut };
                    ...
                    <BreadcrumbList>
                        <BreadcrumbItem>
                            <BreadcrumbPage className="text-gray-700 font-medium">
                                Situation globale des requêtes
                            </BreadcrumbPage>
                        </BreadcrumbItem>
                    </BreadcrumbList>
                </Breadcrumb>
            ...
            <div className="flex flex-1 flex-col gap-6 p-6 bg-gray-50">
                <div className="bg-white rounded-sm shadow-sm border border-gray-200 p-6">
                    ...
                    <form onSubmit={handleSubmitFilters} className="space-y-6">
                        {/* Première ligne - Filtres multiselect */}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <div>
                                <label htmlFor="statut" className="block text-sm font-medium mb-2 text-gray-700">
                                    Statut
                                </label>
                                <select
                                    id="statut"
                            ...
                            <div>
                                <label htmlFor="numero" className="block text-sm font-medium mb-2 text-gray-700">
                                    Numéro
                                </label>
                                <input
                                    type="text"
                        ...
                        {/* Troisième ligne - Référence interne et boutons */}
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <div>
                                <label htmlFor="refinterne" className="block text-sm font-medium mb-2 text-gray-700">
                                    Référence interne
                                </label>
                                <input
                                    type="text"
                            ...
                            <table className="w-full border-collapse">
                                <thead>
                                    <tr className="border-b border-gray-200">
                                        <th className="text-left py-3 px-4 font-medium text-gray-700 text-sm">Projet</th>
                                        <th className="text-left py-3 px-4 font-medium text-gray-700 text-sm">Site</th>
                                        <th className="text-left py-3 px-4 font-medium text-gray-700 text-sm">Numéro</th>
                                        <th className="text-left py-3 px-4 font-medium text-gray-700 text-sm">Objet</th>
                                        <th className="text-left py-3 px-4 font-medium text-gray-700 text-sm">Date soumission</th>
                                        <th className="text-left py-3 px-4 font-medium text-gray-700 text-sm">Demandeur</th>
                                        ...
                                        <th className="text-left py-3 px-4 font-medium text-gray-700 text-sm">Date fin exécution</th>
                                        <th className="text-left py-3 px-4 font-medium text-gray-700 text-sm">Date échéance</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {requetes.length === 0 ? (
                                        ...
                                        requetes.map((requete, index) => (
                                            <tr key={index} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                                                <td className="py-4 px-4 text-sm font-medium text-gray-900">{requete.projetName}</td>
                                                <td className="py-4 px-4 text-sm font-medium text-gray-900">{requete.siteName}</td>
                                                <td className="py-4 px-4 text-sm text-gray-600">{requete.numero}</td>
                                                <td className="py-4 px-4 text-sm text-gray-600 max-w-xs truncate" title={requete.objet}>{requete.objet}</td>
                                                <td className="py-4 px-4 text-sm text-gray-600">{formatDate(requete.dateSoumission)}</td>
                                                <td className="py-4 px-4 text-sm text-gray-600">{requete.demandeur}</td>
                                                <td className="py-4 px-4 text-sm font-medium text-gray-900">{requete.montant}</td>
                                                <td className="py-4 px-4 text-sm">{getStatusBadge(requete.statut)}</td>
                                                <td className="py-4 px-4 text-sm text-gray-600">{formatDate(requete.dateFinExecution)}</td>
                                                <td className="py-4 px-4 text-sm text-gray-600">{formatDate(requete.dateFinEcheance)}</td>
                                            </tr>
                                        ))
                                    )}
                                </tbody>

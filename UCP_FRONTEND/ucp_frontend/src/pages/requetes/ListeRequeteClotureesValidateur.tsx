...
const ListeRequeteClotureesValidateur: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            await fetchRequetes(url1+"/cloturees/page/" + currentPage);
    ...
    useEffect(() => {
        fetchRequetes(url1+"/cloturees/page/" + currentPage);
    }, [currentPage]);
    ...
    const fetchRequetes = async (API_URL: string) => {
        ...
        try {
            const url = debouncedSearchTerm
                ? url1+"/cloturees/word/" + encodeURIComponent(debouncedSearchTerm) + "/page/" + currentPage
                : url1+"/cloturees/page/" + currentPage;
    ...
    const fetchNbRequetesInitiees = async () => {
        ...
        try {
            const url = debouncedSearchTerm
                ? url1+"/cloturees/word/" + encodeURIComponent(debouncedSearchTerm) + "/pages"
                : url1+"/cloturees/pages";
    ...
    const fetchRequetesInitiees = async () => {
        ...
        setError(null);
        try {
            const res = await axios.get<Requete[]>(url1+"/cloturees/page/" + currentPage, {
                withCredentials: true
            });
            setRequetesInitiees(res.data);
            console.log(res.data);
    ...
    const handleExportPDF = () => {
        ...
        exportTableToPDF({
            title: "Liste des Requêtes cloturées",
            headers,
            rows,
            fileName: "liste_requetes_cloturees.pdf"
        });
    };
                            ...
                            <table className="table-auto border-collapse border-1 w-full my-4 ">
                                <thead>
                                    <tr className="text-left text-sm bg-gray-100">
                                        ...
                                        <th className="border-b font-normal text-zinc-500 text-xs p-3 whitespace-nowrap">Projet</th>
                                        <th className="border-b font-normal text-zinc-500 text-xs p-3 whitespace-nowrap">Site</th>
                                        <th className="border-b font-normal text-zinc-500 text-xs p-3 whitespace-nowrap">Réference interne</th>
                                        <th className="border-b font-normal text-zinc-500 text-xs p-3 whitespace-nowrap">Objet de la requête</th>
                                        <th className="border-b font-normal text-zinc-500 text-xs p-3 whitespace-nowrap">Numéro</th>
                                        ...
                                        <th className="border-b font-normal text-zinc-500 text-xs p-3 whitespace-nowrap">Montant</th>
                                        <th className="border-b font-normal text-zinc-500 text-xs p-3 whitespace-nowrap">Début d'activité</th>
                                        <th className="border-b font-normal text-zinc-500 text-xs p-3 whitespace-nowrap">Fin d'activité</th>
                                        <th className="border-b font-normal text-zinc-500 text-xs p-3 whitespace-nowrap">Date fin d'échéance</th>
...
export default ListeRequeteClotureesValidateur; 
...
import { FileText, Loader2, FilePenLine, Upload, Trash2, Eye, GitBranch, GitMerge, CheckCircle, CircleX, Share2, CheckSquare, Undo2, CircleCheck, FileUp, Paperclip, MailPlus, FileDown, Printer, User, Search,FileX } from "lucide-react";
import {
    Tooltip,
    TooltipContent,
    TooltipProvider,
...
const ListeRequeteEnCoursValidateur: React.FC = () => {
    ...
    const fetchNbRequetesVministere = async () => {
        ...
        if (debouncedSearchTerm == "") {
            try {
                const res = await axios.get(url+"/en_cours/pages", {
            ...
            try {
                const res = await axios.get(url+"/en_cours/word/" + debouncedSearchTerm + "/pages", {
                    withCredentials: true
                });
                console.log("page with search");
                console.log(res.data);
                setTotalItems(res.data);
            } catch (error) {
                console.error('Erreur lors du chargement des requêtes:', error);
    ...
    const handlePageSizeChange = (size: number) => {
        ...
        // fetchRequetesVministere(1, size);
    };
    ...
    const fetchRequetesVministere = async () => {
        ...
        if (debouncedSearchTerm == "") {
            try {
                console.log(url + "/en_cours/page/" + currentPage);
                const res = await axios.get<Requete[]>(url+"/en_cours/page/" + currentPage, {
                    withCredentials: true
                });
            ...
            try {
                const res = await axios.get<Requete[]>(url+"/en_cours/word/" + debouncedSearchTerm + "/page/" + currentPage, {
                    withCredentials: true
                });
                setRequetesVministere(res.data);
            } catch (error) {
    ...
    const loadDeletedDocuments = async (id: string) => {
        ...
        try {
            ...
            const res = await axios.get<DeletedAttachment[]>(apiUrl)
        ...
        <>
            ...
            <div className="flex flex-1 flex-col gap-4 p-4 bg-[#fafafa]">
                ...
                <div className="flex justify-between items-center mb-4">
                        ...
                        <h2 className="text-lg font-semibold mb-4 px-4 py-4">Requêtes en circuit</h2>
                            ...
                            <table className="table-auto border-collapse border-1 w-full my-4">
                                <thead>
                                    <tr className="text-left text-sm bg-gray-100">
                                        ...
                                        <th className="border-b font-bold text-black text-xs p-3 whitespace-nowrap" style={{ minWidth: "400px",width:"400px"} }>Objet de la requête</th>
                                        ...
                                        <th className="border-b font-bold text-black text-xs p-3 whitespace-nowrap">Etape</th>
                                        <th className="border-b font-bold text-black text-xs p-3 whitespace-nowrap">En révision</th>
                                        ...
                                        requetesVministere.map((requete) => {
                                            ...
                                            return (
                                                <tr key={requete.idRequete} className="hover:bg-gray-100 cursor-pointer">
                                                    ...
                                                    <td className="border-b  py-2 text-xs text-zinc-500 p-4 "> {requete.objet}</td>
                                                    <td className="border-b  py-2 text-xs text-zinc-500 p-4 whitespace-nowrap">{requete.numRequete}</td>
                                                    <td className="border-b  py-2 text-xs text-zinc-500 p-4 whitespace-nowrap">{/*`${requete.utilisateur.firstname} - ${requete.utilisateur.lastname}`*/requete.numActiviteInterne}</td>
                                                    ...
                                                    <td className="border-b  py-2 text-xs text-zinc-1000 p-4 whitespace-nowrap " >{requete.circuitEtapeCheckListDetailsDTO ? requete.circuitEtapeCheckListDetailsDTO.description : "aucune"}</td>
                                                    <td className="border-b  py-2 text-xs text-zinc-500 p-4 whitespace-nowrap">{requete.manquePj == true ? "oui" : "non"}</td>
...
export default ListeRequeteEnCoursValidateur; 
...
const ListeRequeteManquePj: React.FC = () => {
    ...
    // État pour la recherche avec debouncing
    ...
    const fetchNbRequetesInitiees = async () => {
        ...
        if (debouncedSearchTerm == "") {
            try {
                const res = await axios.get("/Requete/manque_pj/pages", {
                    withCredentials: true
                });
            ...
            try {
                const res = await axios.get("/Requete/manque_pj/word/" + debouncedSearchTerm + "/pages", {
                    withCredentials: true
                });
                setTotalItems(res.data);
    ...
    const fetchRequetesInitiees = async () => {
        ...
        if (debouncedSearchTerm == "") {
            try {
                console.log("/Requete/manque_pj/page/" + currentPage);
                const res = await axios.get<Requete[]>("/Requete/manque_pj/page/" + currentPage, {
                    withCredentials: true
                });
                setRequetesInitiees(res.data);
            ...
            try {
                console.log("/Requete/manque_pj/word/" + debouncedSearchTerm + "/page/" + currentPage);
                const res = await axios.get<Requete[]>("/Requete/manque_pj/word/" + debouncedSearchTerm + "/page/" + currentPage, {
                    withCredentials: true
                });
    ...
    const handleJustificatifsModal = async (requete: Requete, event: MouseEvent<HTMLButtonElement>) => {
        ...
        setDateExecution(requete.dateExecution);
        setDateFinExecution(requete.dateFinExecution);
        ...
        //if (selectedRequete.dateExecution == null) {
            try {
                if (dateExecution == null || dateFinExecution == null) {
                    ...
                    try {
                        console.log(`/Requete/${selectedRequete.idRequete}/date_execution/${dateExecution}/date_fin_execution/${dateFinExecution}`);
                        await axios.put(
            ...
            <div className="flex flex-1 flex-col gap-4 p-4 bg-[#fafafa]">
                ...
                <div className="rounded-sm border bg-card p-4">
                    ...
                    <div className="ml-auto flex gap-2 items-center">
                        <h2 className="text-lg font-semibold mb-4 px-4 py-4">Requêtes en manque de pièces jointes</h2>
                        <div className="ml-auto flex gap-2">
                            <button
                    ...
                    <div className="ml-auto flex gap-2 items-center">
                        <h2 className="text-lg font-semibold mb-4 px-4 py-4">Requêtes à réviser</h2>
                        <div className="ml-auto flex gap-2">
                            <input className="w-1/3 p-1 border rounded-sm ml-auto flex gap-2" type="text" onChange={(e) => handleChangeWord(e)} value={currentWord} />
                            <button
                                className="px-2 py-2 text-sm  bg-green-600 text-white  hover:bg-green-700 flex items-center"
                                onClick={handleExportCSV}
                                disabled={loading || requetesInitiees.length === 0}
                            ...
                            <table className="table-auto border-collapse border-none w-full my-4 ">
                                <thead>
                                    <tr className="text-left text-sm">
                                        ...
                                        <th className="border-b font-bold text-black text-xs">Commentaire de révision</th>
                                    ...
                                    {requetesInitiees.length === 0 ? (
                                        ...
                                        requetesInitiees.map((requete) => (
                                            <tr key={requete.idRequete} className="hover:bg-gray-100 cursor-pointer">
                                                ...
                                                <td className="border-b  py-2 text-xs text-zinc-500">{requete.commentaireRevision}</td>
...
export default ListeRequeteManquePj; 
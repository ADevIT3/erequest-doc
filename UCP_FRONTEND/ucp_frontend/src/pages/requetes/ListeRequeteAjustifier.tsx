...
import { FileText, Loader2, FilePenLine, Upload, Trash2, Eye, FilePlus2, FileDown, Printer, User } from "lucide-react";
import {
    Tooltip,
    TooltipContent,
    TooltipProvider,
...
const ListeRequeteAjustifier: React.FC = () => {
    ...
    const fetchRequetes = async (API_URL: string) => {
        ...
        setError(null);
        try {
            const res = await axios.get<Requete[]>("/Requete/a_justitifer", {
    ...
    const fetchNbRequetesInitiees = async () => {
        ...
        try {
            const url = debouncedSearchTerm
                ? "/Requete/a_justifier/word/" + encodeURIComponent(debouncedSearchTerm) + "/pages"
                : "/Requete/a_justifier/pages";
    ...
    const fetchRequetesInitiees = async () => {
        ...
        try {
            const url = debouncedSearchTerm
                ? "/Requete/a_justifier/word/" + encodeURIComponent(debouncedSearchTerm) + "/page/" + currentPage
                : "/Requete/a_justifier/page/" + currentPage;
    ...
    const handleExportCSV = () => {
        ...
        link.setAttribute("download", "requetes_a_justifier.csv");
    ...
    const handleExportPDF = () => {
        ...
        exportTableToPDF({
            title: "Requêtes à justifier",
            headers,
            rows,
            fileName: "requetes_a_justifier.pdf"
        });
    };
            ...
            <div className="flex flex-1 flex-col gap-4 p-4 bg-[#fafafa]">
                ...
                <div className="rounded-sm border bg-card p-4">
                    ...
                    <div className="ml-auto flex gap-2 items-center">
                        <h2 className="text-lg font-semibold mb-4 px-4 py-4">Requêtes à justifier</h2>
                        <div className="ml-auto flex gap-2">
                            <button
                                ...
                                <tbody>
                                    {requetesInitiees.length === 0 ? (
                                        ...
                                        requetesInitiees.map((requete) => (
                                            ...
                                            requete.isExpired == false ? (<tr key={requete.idRequete} className="hover:bg-gray-100 cursor-pointer">
                                                ...
                                                <td className="border-b  py-2 text-xs text-zinc-500">{requete.utilisateur.agmo.nom}</td>
                                                        ...
                                                        <Tooltip>
                                                            <TooltipTrigger asChild>
                                                                <button
                                                                    ...
                                                                    onClick={() => navigate(`/justificatifs/ajout/${requete.idRequete}`)}
                                                                    disabled={loading}
                                                                >
                                                ...
                                                (
                                                    <tr key={requete.idRequete} className="hover:bg-gray-100 cursor-pointer bg-red-100">
                                                        <td className="border-b font-normal py-2 text-xs text-zinc-1000">{requete.projet.nom}</td>
                                                        <td className="border-b  py-2 text-xs text-zinc-1000 " >{requete.site.nom}</td>
                                                        <td className="border-b  py-2 text-xs text-zinc-500">{/*requete.numActiviteInterne*/ requete.referenceInterne}</td>
                                                        <td className="border-b  py-2 text-xs text-zinc-500 "> {requete.objet}</td>
...
export default ListeRequeteAjustifier; 
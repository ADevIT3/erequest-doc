    ...
    return (
        <>
            ...
            <div className="flex flex-1 flex-col gap-4 p-4 bg-[#f8fafd]">
                ...
                {/* modal confirmation rattachement */}
                {showConfirmationModal && selectedRequeteForCircuit && (
                    <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                        <div className="bg-white rounded-xl shadow-2xl p-6 max-w-lg w-full relative animate-fadeIn scale-100">
                            <label> Voulez-vous envoyer la requête à l’UCP pour validation ?</label>
                            {/* Footer Buttons */}
                            <div className="flex justify-end gap-3 mt-6">
                                ...
                                {!circuitStatus[selectedRequeteForCircuit.idRequete]?.isAttached && (
                                    <button
                                        className="px-4 py-2 rounded-md bg-primary text-white hover:bg-primary/90 transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                        <div className="bg-white rounded-xl shadow-2xl p-6 max-w-lg w-full relative animate-fadeIn scale-100">
                            ...
                            <button
                                ...
                                onClick={() => { setShowCircuitModal(false); setCircuitEtapes([]); }}
                            >
                                ✕
                            </button>
                            ...
                            {circuitStatus[selectedRequeteForCircuit.idRequete]?.isAttached ? (
                                <div className="py-6 text-center bg-amber-50 border border-amber-200 rounded-md">
                                    <p className="text-amber-700 font-medium">
                                        Cette requête est déjà rattachée à un circuit.
                                    </p>
                                </div>
                            ) : (
                                <div className="mb-6">
                                ...
                                {circuitEtapes && circuitEtapes.map((ce, indexCe) => (
                                    ...
                                    >
                                        <p className="font-medium text-gray-900">
                                            Étape {indexCe + 1}: {ce.description}
                                        </p>
                                        <p className="text-xs text-gray-500 mt-1">Validateurs :</p>

                                        <ul className="mt-1 ml-5 list-disc">
                                            {ce.circuitEtapeValidateurs?.map((ceVal, indexCeVal) => (
                                                <li key={indexCeVal} className="text-sm text-gray-700">
                                                    {ceVal.utilisateur.username} —
                                                    <span className="text-gray-500 ml-1">{ceVal.utilisateur.email}</span>
                                                </li>
                                            ))}
                                        </ul>
                                    </div>

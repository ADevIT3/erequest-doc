...
const ListRequeteEnCours: React.FC = () => {
    const [requetesEncours, setRequetesEncours] = useState<Requete[]>([]);
    ...
    // État pour la recherche avec debouncing (amélioration de ListeRequete.tsx)
    const [currentWord, setCurrentWord] = useState("");
    const debouncedSearchTerm = useDebounce(currentWord, 500); // Délai de 500ms

    useEffect(() => {
        fetchNbRequetesEnCours();
    ...
    const fetchNbRequetesEnCours = async () => {
        setLoading(true);
        setError(null);
        if (debouncedSearchTerm == "") {
            try {
                const res = await axios.get("/Requete/en_cours/pages", {
                    withCredentials: true
                });
            ...
            try {
                const res = await axios.get("/Requete/en_cours/word/" + debouncedSearchTerm + "/pages", {
                    withCredentials: true
                });
                setTotalItems(res.data);
    ...
    const fetchRequetesInitiees = async () => {
        ...
        if (debouncedSearchTerm == "") {
            try {
                console.log("/Requete/en_cours/page/" + currentPage);
                const res = await axios.get<Requete[]>("/Requete/en_cours/page/" + currentPage, {
                    withCredentials: true
                });
                setRequetesEncours(res.data);
            ...
            try {
                console.log("/Requete/en_cours/word/" + debouncedSearchTerm + "/page/" + currentPage);
                const res = await axios.get<Requete[]>("/Requete/en_cours/word/" + debouncedSearchTerm + "/page/" + currentPage, {
                    withCredentials: true
                });
    ...
    const handleExportCSV = () => {
        ...
        const data = requetesEncours.map(r => [
            r.idRequete,
            r.projet.nom,
            r.site.nom,
        ...
        link.setAttribute("download", "requetes_en_cours.csv");
    ...
    const handleExportPDF = () => {
        ...
        const rows = requetesEncours.map(r => [
            r.idRequete,
            r.projet.nom,
            r.site.nom,
            r.numActiviteInterne,
        ...
        exportTableToPDF({
            ...
            headers,
            rows,
            fileName: "requetes_en_cours.pdf"
        });
    };
            ...
            <div className="flex flex-1 flex-col gap-4 p-4 bg-[#fafafa]">
                ...
                <div className="rounded-md border bg-card p-4">
                    <div className="flex justify-between items-center mb-4">
                        <h2 className="text-lg font-semibold mb-4 px-4 py-4">Requêtes en cours de validation</h2>
                        <div className="flex items-center gap-4">
                            ...
                            <button
                                ...
                                disabled={loading || requetesEncours.length === 0}
                            >
                                    ...
                                    {requetesEncours.length === 0 ? (
                                        ...
                                        requetesEncours.map((requete) => (
                                            <tr key={requete.idRequete} className="hover:bg-gray-100 cursor-pointer">
                                                ...
                                                <td className="border-b  py-2 text-xs text-zinc-1000 " >{requete.circuitEtapeCheckListDetailsDTO ? requete.circuitEtapeCheckListDetailsDTO.description : "aucune"}</td>
...
export default ListRequeteEnCours; 
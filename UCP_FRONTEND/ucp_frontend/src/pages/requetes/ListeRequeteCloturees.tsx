...
const ListeRequeteCloturees: React.FC = () => {
    ...
    // États pour la recherche avec debouncing (amélioration de ListeRequete.tsx)
    const [currentWord, setCurrentWord] = useState("");
    const debouncedSearchTerm = useDebounce(currentWord, 500); // Délai de 500ms
    ...
    // useEffect pour la recherche avec debouncing (amélioration de ListeRequete.tsx)
    useEffect(() => {
        fetchNbRequetesCl();
        fetchRequetesInitiees();
    }, [debouncedSearchTerm]);
    ...
    const fetchNbRequetesCl = async () => {
        ...
        if (debouncedSearchTerm == "") {
            try {
                const res = await axios.get("/Requete/cloturees/pages", {
            ...
            try {
                const res = await axios.get("/Requete/cloturees/word/" + debouncedSearchTerm + "/pages", {
                    withCredentials: true
                });
                setTotalItems(res.data);
    ...
    const fetchRequetesInitiees = async () => {
        ...
        if (debouncedSearchTerm == "") {
            try {
                console.log("/Requete/cloturees/page/" + currentPage);
                const res = await axios.get<Requete[]>("/Requete/cloturees/page/" + currentPage, {
                    withCredentials: true
                });
                setRequetesInitiees(res.data);
            ...
            try {
                console.log("/Requete/cloturees/word/" + debouncedSearchTerm + "/page/" + currentPage);
                const res = await axios.get<Requete[]>("/Requete/cloturees/word/" + debouncedSearchTerm + "/page/" + currentPage, {
                    withCredentials: true
                });
    ...
    // Fonction de gestion du changement de recherche (identique à ListeRequete.tsx)
    ...
    const fetchRequetesVministere = async () => {
        ...
        setError(null);
        try {
            const res = await axios.get<Requete[]>("/Requete/cloturees", {
                withCredentials: true
            });
    ...
    const handleExportValidationCSV = () => {
        ...
        const data = validationHistory.map((item: any) => [
            item.intituleEtape || "—",
            item.validateur || "—",
            item.dateValidation || "—",
            item.commentaire || "—"
            ...
            <div className="flex flex-1 flex-col gap-4 p-4 bg-[#fafafa]">
                ...
                {showValidationHistoryModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/25 backdrop-blur-[2px]">
                        <div className="bg-white rounded-sm shadow-lg p-4 max-w-4xl w-full relative">
                            <button
                                className="absolute top-2 right-2 text-gray-500 hover:text-gray-700"
                                ...
                                <table className="table-auto border-collapse border w-full">
                                    <thead>
                                        <tr className="text-left text-sm">
                                            <th className="border font-normal text-zinc-500 text-xs">Étape</th>
                                            <th className="border font-normal text-zinc-500 text-xs">Validateur</th>
                                            <th className="border font-normal text-zinc-500 text-xs">Date</th>
                                            <th className="border font-normal text-zinc-500 text-xs">Commentaire</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {validationHistoryLoading ? (
                                            ...
                                            validationHistory.map((item: any, index: number) => (
                                                <tr key={index} className="hover:bg-gray-100">
                                                    <td className="border py-2 text-xs text-zinc-1000">{item.intituleEtape || "—"}</td>
                                                    <td className="border py-2 text-xs text-zinc-1000">{item.validateur || "—"}</td>
                                                    <td className="border py-2 text-xs text-zinc-500">{item.dateValidation || "-"}</td>
                                                    <td className="border py-2 text-xs text-zinc-500">{item.commentaire || "—"}</td>
                                                </tr>
                                            ))
                                        )}
                                    </tbody>
                ...
                <div className="rounded-sm border bg-card p-4">
                    <div className="ml-auto flex gap-2 items-center">
                        <h2 className="text-lg font-semibold mb-4 px-4 py-4">Requêtes cloturées</h2>
                        <div className="ml-auto flex gap-2 items-center">
                            <input
                            ...
                            <table className="table-auto border-collapse border-none w-full my-4 ">
                                <thead>
                                    <tr className="text-left text-sm">
                                        ...
                                        <th className="border-b font-bold text-black text-xs">Début d'activité</th>
                                        <th className="border-b font-bold text-black text-xs">Fin d'activité</th>
                                        <th className="border-b font-bold text-black text-xs">Date fin d'échéance</th>
                                    ...
                                    {requetesInitiees.length === 0 ? (
                                        ...
                                        requetesInitiees.map((requete) => (
                                            <tr key={requete.idRequete} className="hover:bg-gray-100 cursor-pointer">
                                                ...
                                                <td className="border-b  py-2 text-xs text-zinc-500">{new Date(requete.dateFinExecution).toLocaleDateString()}</td>
                                                <td className="border-b  py-2 text-xs text-zinc-500">{new Date(requete.dateFinEcheance).toLocaleDateString()}</td>
...
export default ListeRequeteCloturees; 
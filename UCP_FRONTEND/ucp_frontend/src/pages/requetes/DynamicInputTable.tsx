...
export default function DynamicInputTable({ types, projets, sites, typesRequete, units, activites: initialActivites, onSubmit }: DynamicInputTableProps) {
    ...
    const fetchVilles = async () => {
        try {
            const response = await apiFetch('Distance/villes', {
                method: 'GET',
                credentials: 'include'
            });
            if (response.ok) {
    ...
    const calculateDistance = async (depart: string, arrivee: string): Promise<number | null> => {
        if (!depart || !arrivee || depart === arrivee) {
            console.log("Villes identiques ou manquantes");
            return null;
        }
        
        try {
            console.log("Calcul de la distance entre", depart, "et", arrivee);
            const response = await apiFetch(
                `Distance?from=${encodeURIComponent(depart)}&to=${encodeURIComponent(arrivee)}`,
                {
                    method: 'GET',
                    credentials: 'include'
                }
            ...
            if (!response.ok) {
                console.error('Erreur API lors du calcul de la distance');
                return null;
            }
            
            const data = await response.json();
            console.log("Distance reçue de l'API:", data);
    ...
    function evaluateFormula(formula: string, scope: Record<string, any>) {
        ...
        // Si c'est un calcul de distance, on le gère séparément
        if (formula.includes('calculDistance')) {
            return null;
        }
        ...
        // Fonction principale pour mettre à jour une ligne avec calcul de distance
const updateRowWithFormula = async (
    sectionIndex: number,
    tableIndex: number,
    rowIndex: number,
    columnId: string,
    value: string,
    categorieColonnes: CategorieRubriqueColonne[]
) => {
    console.log("🚀 updateRowWithFormula appelé", { columnId, value });
    
    // 1. D'abord trouver quelle colonne est modifiée
    const targetColumn = sections[sectionIndex]?.tables[tableIndex]?.rows[rowIndex]?.columns
        .find(c => c.idColumn === columnId);
    
    console.log("📝 Colonne modifiée:", targetColumn?.nameColumn);
    
    // 2. Mettre à jour la valeur IMMÉDIATEMENT
    setSections(prev => {
        const updated = [...prev];
        const table = updated[sectionIndex].tables[tableIndex];
        const row = table.rows[rowIndex];
        ...
        const columnToUpdate = columns.find((c) => c.idColumn === columnId);
        if (columnToUpdate) {
            columnToUpdate.columnValue = value;
          //  console.log(✅${ columnToUpdate.nameColumn } = ${ value });
        }

        // Construire le scope pour les formules
        const rowForFormula = Object.fromEntries(
            columns.map(c => [
                c.nameColumn.trim(),
                isStringifiedNumber(c.columnValue) ? Number(c.columnValue) : c.columnValue
            ])
        );

        // Recalculer toutes les colonnes qui ont une formule
        for (let i = 0; i < columns.length; i++) {
            const categorieColonne = categorieColonnes.find(
                cc => cc.idCategorieRubriqueColonne.toString() === columns[i].idColumn.toString()
            );
            if (categorieColonne?.formule != null) {
                console.log("PINGGGGG " + categorieColonne.nom);
                console.log(categorieColonne.formule);
                const result = evaluateFormula(categorieColonne.formule, rowForFormula);
                if (result !== null) {
                    columns[i].columnValue = result.toString();
                    // Mettre à jour le scope pour les formules suivantes
                    rowForFormula[columns[i].nameColumn.trim()] = Number(result) || result;
                }
            }
        }
    ...
    // 3. Si c'est une colonne Départ ou Arrivée, attendre et calculer la distance
    const targetCategorieColonne = categorieColonnes.find(
    c => c.idCategorieRubriqueColonne.toString() === columnId.toString()
);
    const targetRole = targetCategorieColonne?.role;

    if (targetRole === "depart" || targetRole === "arrivee") {
        console.log("📍 C'est une ville, on attend pour calculer la distance...");
        
        // Attendre 100ms pour que React mette à jour l'état
        setTimeout(async () => {
            // Utiliser setSections avec callback pour avoir l'état frais
            setSections(current => {
                const updated = [...current];
                const table = updated[sectionIndex].tables[tableIndex];
                const row = table.rows[rowIndex];
                
                // Voir toutes les colonnes pour debug
                console.log("🔍 Colonnes dans la ligne:", 
                    row.columns.map(c => `${c.nameColumn}: "${c.columnValue}"`)
                );
                
                // Trouver les colonnes Départ et Arrivée
             const departCol = row.columns.find(c => {
    const cat = categorieColonnes.find(
        cc => cc.idCategorieRubriqueColonne.toString() === c.idColumn.toString()
    );
    return cat?.role === "depart";
});

const arriveeCol = row.columns.find(c => {
    const cat = categorieColonnes.find(
        cc => cc.idCategorieRubriqueColonne.toString() === c.idColumn.toString()
    );
    return cat?.role === "arrivee";
});

const distanceCol = row.columns.find(c => {
    const cat = categorieColonnes.find(
        cc => cc.idCategorieRubriqueColonne.toString() === c.idColumn.toString()
    );
    return cat?.role === "distance";
});
                
                console.log("📍 Départ:", departCol?.columnValue, "Arrivée:", arriveeCol?.columnValue);
                console.log("📏 Colonne Distance trouvée:", distanceCol?.nameColumn);
                
                if (departCol && arriveeCol && distanceCol && 
                    departCol.columnValue && arriveeCol.columnValue) {
                    console.log("📍 Les deux villes sont sélectionnées!");
                    
                    // Lancer le calcul de distance
                    calculateDistance(departCol.columnValue, arriveeCol.columnValue)
                        .then(distance => {
                            if (distance !== null) {
                                console.log("📏 Distance calculée:", distance);
                                
                                // Mettre à jour la distance dans un NOUVEL état
                                setSections(prevState => {
                                const newUpdated = [...prevState];
                                const newTable = newUpdated[sectionIndex].tables[tableIndex];
                                const newRow = newTable.rows[rowIndex];

                                // ✅ Recherche par rôle, avec comparaison toString()
                                const newDistanceCol = newRow.columns.find(c => {
                                    const cat = categorieColonnes.find(
                                        cc => cc.idCategorieRubriqueColonne.toString() === c.idColumn.toString()
                                    );
                                    return cat?.role === "distance";
                                ...
                                if (newDistanceCol) {
                                    newDistanceCol.columnValue = distance.toString();
                                    console.log("✅ Distance mise à jour:", distance);
                                } else {
                                    console.error("❌ Colonne Distance non trouvée!");
                                }
...
// Fonction pour recalculer les formules
    const recalculateFormulas = (row: { columns: ColumnItem[] }, categorieColonnes: CategorieRubriqueColonne[]) => {
        const rowForFormula = Object.fromEntries(
            row.columns.map(c => [
                c.nameColumn.trim(),
                isStringifiedNumber(c.columnValue) ? Number(c.columnValue) : c.columnValue
        ...
        // Pour chaque colonne qui a une formule, recalculer
        row.columns.forEach((col, index) => {
            const categorieColonne = categorieColonnes[index];
            if (categorieColonne.formule && categorieColonne.nom !== "Distance (km)") {
                const result = evaluateFormula(categorieColonne.formule, rowForFormula);
                if (result !== null) {
                    col.columnValue = result.toString();
                }
            }
        });
    };

    function isStringifiedNumber(value: any) {
    ...
    useEffect(() => {
    // Charger les villes depuis votre API
    const fetchVilles = async () => {
        try {
            const response = await apiFetch('Distance/villes', {
                method: 'GET',
...
{row.columns.map((col, colIndex: number) => {
    // Trouver le rôle de la colonne via table.columns
    const categorieColonne = table.columns.find(
        tc => tc.idCategorieRubriqueColonne.toString() === col.idColumn.toString()
    );
    const colRole = categorieColonne?.role;

    return (
        col.nameColumn != "Total_valide" ? (<td key={colIndex} className="border p-2">
            {col.nameColumn === "Unit" ? (
                <select
                    value={col.columnValue || ""}
                ...
                // Colonne distance : affichage en lecture seule
                <span className="font-bold">{formatNumber(col.columnValue)} km</span>

            ) : colRole === "depart" || colRole === "arrivee" ? (
                // Colonne ville : select avec liste des villes
                <select
                    value={col.columnValue || ""}
                    onChange={(e) => updateRowWithFormula(sectionIndex, tableIndex, rowIndex, col.idColumn, e.target.value, table.columns)}
                    className="border p-1 w-full rounded"
                    ...
                    onChange={(e) => {
                        ...
                        if (col.datatype === "nombre") {
                            ...
                            if (/^\d*\.?\d{0,2}$/.test(normalized) || normalized === "") {
                                updateRowWithFormula(sectionIndex, tableIndex, rowIndex, col.idColumn, normalized, table.columns);
                            }
                        } else {
                            updateRowWithFormula(sectionIndex, tableIndex, rowIndex, col.idColumn, value, table.columns);
                        }
                    }}
                    className="border-0 w-full h-8 rounded"
                    required

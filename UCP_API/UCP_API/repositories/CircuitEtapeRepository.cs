...
{
    public class CircuitEtapeRepository
    {
        ...
        public List<CircuitEtape> GetCircuitEtapesByCircuit(int idCircuit)
        {
            return _context.CircuitEtape.FromSqlRaw("select *  from circuitetape where idcircuit = @p0 and deletiondate is null", idCircuit).Include(c => c.CircuitEtapeValidateurs).ToList();
        }
